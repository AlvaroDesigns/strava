...
interface ElevationChartGradientProps {
  data: Array<{ date: string; [key: string]: string | number }>;
  users: Array<{ id: string; name: string; key: string }>;
}

export function ElevationChartGradient({ data, users }: ElevationChartGradientProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Desnivel Medio</CardTitle>
        <CardDescription>
          Mostrando desnivel medio por usuario en el tiempo
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            {users.map((user, index) => {
              ...
              return (
                <Area
                  ...
                  dataKey={user.key}
                  type="natural"
                  fill={`url(#fillElevation${user.key})`}
                  fillOpacity={0.4}
                  stroke={color}
                  stackId="a"
                />
